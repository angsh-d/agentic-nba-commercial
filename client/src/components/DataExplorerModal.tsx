...
export function DataExplorerModal({ open, onOpenChange }: DataExplorerModalProps) {
  ...
  const { data: tableData, isLoading: dataLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/database/tables/${selectedTable}?limit=10000`);
      if (!response.ok) throw new Error("Failed to fetch table data");
      return response.json();
    },
    enabled: !!selectedTable && open,
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] h-[90vh] flex flex-col p-0">
        <DialogHeader className="px-6 py-4 border-b border-gray-200">
          <DialogTitle className="text-2xl font-semibold text-gray-900">Database Explorer</DialogTitle>
          <p className="text-sm text-gray-500 font-light">View all data in NeonDB PostgreSQL database</p>
        </DialogHeader>
                  ...
                  <div className="px-6 py-4 border-b border-gray-200 bg-white flex items-center justify-between sticky top-0 z-10">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {tableData?.rows?.length || 0} records {tableData?.rows?.length === 10000 ? "(limit reached)" : "(all data)"}
                      </p>
                    </div>
                    <Button
                      variant="outline"
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {tableData.rows.map((row: any, idx: number) => (
                            <tr key={idx} className="hover:bg-gray-50">
                              {tableData.columns.map((col: string) => (
                                <td key={col} className="px-6 py-4 text-sm text-gray-900 max-w-md">
